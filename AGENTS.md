...
- **Still open: `src/worker/models.rs:30/:209`** — `list_models` transiently returns nothing and caches the empty result (the on-demand catalogue one-shot guards its answers the same way: an empty one replaces nothing).
