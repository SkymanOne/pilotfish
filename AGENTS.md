...
| `src/fleet/run.rs` | `RunState` (stored as `runs/<id>/run.json`, camelCase on disk, serde tolerant, and the `uuid` field is the run's identity), `RunStatus`, derived status/view (30 s starting grace, `kill(pid,0)` liveness with EPERM-alive), `find_run` (exact id → uuid → alias → legacy `<name>-<14-digit>` form, where several live runs sharing an alias is an error naming the candidates), `list_runs`/`list_runs_for_owner`, steering log (capped at 20), `THINKING_LEVELS`. |
...
| `src/worker/` | the detached worker monitor (`monitor.rs`), pi RPC message types (`rpc.rs`), `pi --list-models`, model checking and the on-demand catalogue one-shot (`models.rs`). The monitor also materialises the embedded pi extension and skill into `.pilotfish/pi/` at boot. |
...
| `src/desktop/` | `pilotfish desktop`, behind the `desktop` feature. `backend.rs` runs a `Driver` on the tokio runtime plus what only the window shows (every session, every session's runs for the Board, the selected worker's `patch::load`) and publishes an `Arc<Snapshot>` on a `watch` channel after every change, and the window sends `UiCmd`s on an `mpsc`. The GPUI half never reads a file or runs git: `main_view.rs` (panes, resize/fold, composer with completions, and the buttons, menus and dialogs that stand in for commands, each sending the same `/command` line or `UiCmd` a keyboard user would), `chat.rs`, `changes.rs` (the diff pane), `board.rs` (the ⌘B popup), `sheets.rs` (one sheet per `Overlay`), `keys.rs` (GPUI keystroke → crossterm `KeyEvent`, so overlays read keys through `Console::handle_key` exactly as in the TUI), `theme.rs` (the two palettes, fonts, and the `AssetSource` serving the logo), `ui.rs` (the shared pieces: sheet, pill, chip, logo, the wake bar and the two motions). `shots.rs` is test tooling, only under `desktop-shots`. |
...
Tools, all thin wrappers over the same `ops` functions the CLI subcommands call: `fleet_spawn`, `fleet_status`, `fleet_wait`, `fleet_output`, `fleet_logs`, `fleet_send`, `fleet_followup`, `fleet_answer`, `fleet_stop`, `fleet_report`, `fleet_diff`, `fleet_merge`, `fleet_cleanup`. Every result ends with an `exit: N` line carrying the CLI exit code, so the agent branches on the same numbers a script would. `fleet_merge` refuses (exit 1) while the human has a merge, cherry-pick, revert, rebase or unresolved conflicts in progress, aborts on conflict (exit 5) and names the base commit: conflicts go back to the worker as a rebase brief, because the orchestrator cannot edit files. `fleet_spawn` refuses a `session` that names a pi session file whose recorded working directory no longer exists (the usual mark of `fleet_cleanup` having removed the old run's worktree), or a path that names no file (exit 1, before anything is created). A bare session id is left to pi. `fleet_status` falls back to the recorded `session_arg` when a run's own `session/` dir is empty, since pi keeps writing a resumed session into the original file. `fleet_wait` defaults to 120 s with `timeoutSec` validated 1..=600, where the CLI's default is 600 s. With routing on, `fleet_spawn` can block for up to ten minutes while the human chooses a model Jev was unsure of (see Routing a brief), and the prompt tells the agent that wait is expected.
...
| Data | `serde`, `serde_json`, `time`, `uuid` | on-disk JSON is camelCase and tolerant of unknown/missing fields, and `uuid` is the identity type for runs and sessions |
...
| Config edits | `toml_edit` | `set_routing` edits `[routing]` keys in `~/.pilotfish/config.toml` in place, keeping the user's comments and order, while `toml` still does the reading |
...
- `can_use_tool` arrives without any `initialize` handshake (`echo` never prompts, being in the built-in read-only set, so probe with a writing command). The handshake is sent at startup anyway: its response carries the command/skill list, and a bare `{subtype:"initialize"}` is acked but not needed.
...
- Permission modes: the flag accepts acceptEdits, auto, bypassPermissions, manual, dontAsk, plan and `default` (a hidden alias for `manual`, while `bogus` is rejected), and `set_permission_mode` succeeds for all of them, so the modes the console offers work both at launch and mid-session.
...
.pilotfish/
  fleet.json            the v2 session store, `{"version":2,"sessions":{<uuid>: row}}`. Each row: alias,
                        ...
                        Rows are only ever created by the user (`/session new`). A monitor updates just
                        its own fields of an existing row (session id, pids, model, launch) and never
  ...
  pi-cache.json         the fleet-level pi catalogue (models + commands) with `fetchedAt`, a property of
                        ...
                        Written by a worker monitor at boot, and fetched on demand from a one-shot
                        pi ("pi --mode rpc --no-session") when the console or a routed spawn needs it
  orchestrators/        one directory per session, created lazily
  orchestrators/<alias|-default>-<short-uuid>/   named at creation. A rename leaves it, and
                        `FleetPaths::orchestrator_dir` falls back to the dir ending in the short uuid
...
`inbox.jsonl`, `outbox.jsonl` and `session/` appear only on first use, so a run that is never steered has no `inbox.jsonl` on disk. Status is derived, never stored: a run whose monitor is gone reads as `dead`, a running worker waiting on `fleet_ask` or a pi dialog reads as `blocked`. Gone for good: top-level `reports/`, `orchestrator.json`, per-run `monitor.log`, `tui.lock`. There is no migration, and an old `.pi-fleet` is ignored.
...
- Both files carry `fetchedAt`. The console refreshes anything older than 30 s when the palette opens or an unknown `/command` is typed: `OrchestratorCommand::RefreshCapabilities` re-issues the `initialize` request, and the `refresh_capabilities` envelope makes a live worker monitor re-ask pi (`get_state`, `get_commands`, `get_available_models`). With no worker running, the console asks pi itself: `Effect::FetchPiCatalogue` starts a one-shot pi (`--mode rpc --no-session`, the same materialised extension and skill a worker boots with, cwd at the repo root) off the UI loop. The palette and the `/routing` panel both trigger it, at most one runs at a time, and the feed tick collects the result without the console ever awaiting the fetch.
...
- A worker monitor updates `pi-cache.json` one field at a time (read, change the field that arrived, write) under the `pi-cache.json.lock` sidecar (`run::update_pi_cache`), never from a whole cache in its memory: monitors boot side by side and learn commands and models in separate replies, and a whole-file write after the first once wiped the models another spawn was routing from. The on-demand one-shot writes the same way, and an empty models answer replaces nothing. A command list pi only picks up after `/reload` in a *running* process is what the one-shot's fresh process is for: it sees newly installed commands without starting a worker.
...
- **An overlay that is a list reads single letters as commands** (`map_overlay_key`), and one that is a text field reads them as text (`map_key`). `Console::handle_key` picks per overlay, including the permission overlay, which switches while a deny reason or a custom answer is being written, because a reason starting with "just" must not lose its letters to list navigation. `n` and `y` are never remapped, because that is how a confirm prompt hears no.
...
- **A model question raises itself too** (`Overlay::ModelChoice`), under the same rules as a permission prompt and after any pending one not yet raised. A permission prompt raised once and dismissed does not keep it down: the spawn that asked it is waiting, and so is the orchestrator's `fleet_spawn`. The runtime reads `routing::pending_questions` on the feed tick. A question that vanishes (answered, expired, asker gone) closes its prompt. `esc` leaves it pending, the status line counts it, and `a` on the orchestrator row reopens it.
- **`/routing` (`/jev`)** opens `Overlay::Routing`: the switch, where the key in force comes from, what routing would choose between or why it would decline, the shortlist size and the ask limit (`-`/`+`, steps of 0.05). A key being entered is a `Secret` drawn one dot per character, and saving writes `[routing] api_key`. The status is read synchronously and only while the panel is open, and a reload never starts a catalogue fetch. Only opening the panel, the palette or `m` does, since a finished fetch reloads the status and a reload that fetched once went round every feed tick. `m` opens the shortlist editor (`ShortlistEditor`), a text field over `pi-cache.json`: typing filters, `enter` ticks, `esc` saves `[routing] models`. Bare-id entries expand to the `provider:id` rows they match, and entries that match nothing are kept verbatim. When the cache is missing or stale, opening the panel and `m` fetch it from pi in the background (a real refresh, which restamps `fetchedAt`) (`m` says it is fetching instead of claiming there is no catalogue).
- **Folding.** Reasoning and tool output older than `RECENT_BLOCKS` fold to one summary row each, and `/verbose` (`ctrl-o`) unfolds them. The model's prose, the human's prompts, fleet events and errors are never folded.
...
- **Block indices are rebased, not left to rot.** `Transcript::dropped()` counts blocks that fell off the top past `MAX_BLOCKS`, and `Console::rebase_scroll` moves a pinned scroll and the search hits by the same amount, so a long session does not silently shift what the reader is looking at. Paging still moves in blocks rather than rendered rows.
...
- **Per-session shutdown.** Removing `orchestrators/<key>/` stops exactly that monitor within `MISSING_DIR_POLLS` polls, and deleting `.pilotfish` stops them all.
- **Monitor health.** `last_heartbeat` is stamped on a 5 s cadence (`HEARTBEAT_WRITE_MS`), and `monitor_health` derives Running / Wedged / Stopped from heartbeat freshness (`HEARTBEAT_GRACE_MS` 15 s) plus pid liveness. A live pid with a stale heartbeat is a wedged monitor.
...
- **Candidates first.** `narrow` cuts pi's catalogue to the pinned provider (`--provider` or `[worker] provider`) and the `[routing] models` shortlist (`provider:id`, or a bare id for every provider). Every option is named `provider:id`, because most ids are served by several providers that do not behave alike (verified facts 4 and 6). More than `MAX_CHOICES` (255, TypeSafe's limit) and routing declines with a note saying how to narrow it. The shortlist editor refuses a 256th entry for the same reason.
...
- **Declines are said, not swallowed.** Routing on but no key, no store, an unaskable pi, too many candidates, or no answer: the spawn goes ahead on the configured defaults and prints `routing: not routed: …`. A judgment that cannot be had is never an error. An empty or missing catalogue is not a decline: `route_with_key` fetches it from pi on demand (one-shot, bounded, never an error) before building the brief, so a fresh fleet routes straight away.
- **The seam** is `route_request` in `src/ops/spawn.rs`. The cap is checked twice (`cap_refusal`): once before routing, so a full session never pays for a judgment or asks the human, and once after routing *and* `check_model` (`pi --list-models`, up to 10 s), right before `create_run`, so nothing slow sits between the count that decides and the run it admits. The key is only looked up once routing is on, because reading the store can raise a dialog. `route_with_key` is the part below the lookup, which is what the tests drive. What was decided is written to `run.json` as `routing`, its `note` saying who chose.
- **The key** lives in `src/secrets.rs`: `$TYPESAFE_API_KEY`, then `[routing] api_key` in `~/.pilotfish/config.toml`, which the console writes when the human pastes a key (asked for at start-up when routing is on without one, on `/routing`, and on switching routing on). `paths::set_routing` writes that file by rename from a mode-600 temp file, so no reader sees it half-written and the key is never on disk with wider permissions. The OS credential store (`keyring`) was dropped on 2026-09-24: on macOS every rebuilt, ad-hoc-signed binary is a new app to the Keychain, so each `cargo install` or test build put up a password dialog, and a console loop then read it every feed tick. `Secret` (with its own `Deserialize`) keeps the key out of every `Debug`, so effects, states and the parsed config stay safe to log.
- **Config:** `[routing] enabled`, `model` (`jev-latest`), `confidence_threshold` (validated 0..1 at load, so `80` is an error, not a silent default), `endpoint`, `models`. `paths::set_routing` edits any of them in place with `toml_edit`. `$PILOTFISH_TYPESAFE_URL` or `endpoint` points it elsewhere, which is how the tests reach a local stub (`route::test_support::stub`, one reply per request). `--route` / `--no-route` and `fleet_spawn`'s `route` override `enabled` per spawn.
...
- The orchestrator monitor used to upsert its whole in-memory row on every save. A removed session came back (under its old name) the moment its monitor saved on the way out, and a rename or the console's watcher cursors were overwritten by the monitor's stale copy. `Monitor::save_record` now updates only the monitor's own fields of a row that still exists. A monitor booted with no row writes one once, at boot.
- The console used to create a session whenever it found none: on first open, and after the last one was removed. Sessions are now only started by the user. `driver::resolve_console_key` returns `None` rather than creating one, and the `Driver` runs without a poll until a session is open.
...
Tests never resolve an ambient `PILOTFISH_DIR`: `FleetPaths::discover` prefers `$PILOTFISH_DIR` over `<cwd>/.pilotfish`. That is right for production, but a bare `cargo test` inside a live fleet (the monitor exports `PILOTFISH_DIR`) once operated on the real fleet. Resolution is therefore injectable end to end (`FleetPaths::discover_with_env`, `resolve_fleet_dir_with_env`, `resolve_run_with_env`, the `*_core_with_env` twins in `ops/`, `FleetServer::with_pilotfish_dir`), with public forms delegating to the ambient value and tests passing `None`. Every test that spawns the binary pins `PILOTFISH_DIR` per child with `Command::env`, or removes it where the `<cwd>/.pilotfish` fallback is the point under test (`tests/console_refusal.rs`). `std::env::set_var` is `unsafe` in edition 2024 and `unsafe_code` is forbidden crate-wide, so per-child env is the only tool. Regression: `spawn_isolation` in `tests/cli_e2e.rs` proves a spawned `pilotfish` writes only into the fleet dir it was given.

Tests never read the developer's `~/.pilotfish` either: it can switch routing on and hold a real key, and a spawn under it would call the real TypeSafe API. Every child that could spawn gets `PILOTFISH_HOME` pinned to an empty directory (`NO_USER_HOME` in `tests/cli_e2e.rs`, the same path in `tests/mcp_stdio.rs` and `tests/orch_multi_session.rs`), and the console's unit tests point `Console::user_dir` at a temporary directory.

Tests are grouped by concern, one test per behaviour area with a short name (`force_stop_worker_process`, `merge_cases`), and each merged test keeps its cases as separate blocks. Constants, derived defaults and one-line helpers are not tested on their own. They are covered by the behaviour that uses them. `cargo llvm-cov` is the check that a trim loses nothing: on 2026-09-24 the suite went from 594 tests to 188 with no production line losing coverage (lines covered only by spawned monitors vary run to run, so compare the union of two runs).
...
| `TYPESAFE_API_KEY` | the routing key. It wins over `[routing] api_key`, and with neither routing is inert |
...
| `PILOTFISH_SHOTS` | a directory. With the `desktop-shots` feature, the window runs the `script` lines appended there (`shot`, `key`, `type`, `click`, `drag`, `scroll`, `board`, `wait`) and renders `shot`s offscreen to PNG, drawing a frame after every step since a hidden or occluded window gets none of its own. No Screen Recording or Accessibility permission is involved, which is how the UI is checked from a background session |
