...
| `src/route.rs` | choosing a worker's model, thinking level and worktree from its brief with TypeSafe's System One (Jev). Off by default; a judgment that cannot be had is no judgment, never an error. `narrow` is the one definition of which catalogue models matter, shared with `fleet_status`. |
...
| Config edits | `toml_edit` | `set_routing_enabled` edits `~/.parl/config.toml` in place, keeping the user's comments and order; `toml` still does the reading |
...
- **`/routing` (`/jev`)** opens `Overlay::Routing`: the switch, where the key in force comes from, and what routing would choose between or why it would decline. A key being entered is a `Secret` drawn one dot per character; saving goes to the OS store off the UI thread. Status loads the same way, since reading the store can raise a dialog.
...
- **Candidates first.** `narrow` cuts pi's catalogue to the pinned provider (`--provider` or `[worker] provider`) and the `[routing] models` allowlist (`provider:id`, or a bare id for every provider). Every option is named `provider:id` — most ids are served by several providers that do not behave alike (verified facts 4 and 6). More than `MAX_CHOICES` (255, TypeSafe's limit) and routing declines with a note saying how to narrow it.
- **One request, four questions**, because they do not depend on one another: a Choice over the candidates (each described with its context window, cost and reasoning levels), a Score for effort, and two Nouls (`needs_worktree`, `parallel_safe`). `POST /v1/systemone`, `Authorization: Bearer …` — there is no Rust SDK, so this speaks the documented API directly.
- **Code owns the policy.** Below `[routing] confidence_threshold` (0.6) the configured model stands. The effort Score is clamped to the levels of the model that will *actually run* — the chosen one, or the fallback when the choice was not taken — and a model with no levels gets none asked for (verified fact 5). A worktree is only dropped when `needs_worktree` is under `READ_ONLY_BELOW` (0.1): being wrong that way puts a worker's edits in the human's checkout. A pinned `model` and `thinking` together are never routed.
...
- **The seam** is `route_request` in `src/ops/spawn.rs`, run *before* the per-session cap is counted, so a slow judgment cannot widen the gap between counting live workers and creating the new one. The key is only looked up once routing is on — reading the store can raise a dialog. What was decided is written to `run.json` as `routing`.
...
- **Config:** `[routing] enabled`, `model` (`jev-latest`), `confidence_threshold` (validated 0..1 at load — `80` is an error, not a silent default), `endpoint`, `models`. `$PARL_TYPESAFE_URL` or `endpoint` points it elsewhere, which is how the tests reach a local stub. `--route` / `--no-route` and `fleet_spawn`'s `route` override `enabled` per spawn.
