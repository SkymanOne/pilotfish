...
| `ctrl-r` | Search the session. Press again for the next match |
...
| `/shutdown` | `/sd` | Stop the orchestrator and every worker, then exit. Asks first, and keeps worktrees and branches |
...
Most terminals also bypass mouse capture while a modifier is held during a drag, which needs no toggle: `option` in iTerm2 and Terminal.app, `shift` in kitty, Ghostty and WezTerm.
