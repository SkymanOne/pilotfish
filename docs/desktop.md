...
* **Workers**: the open session's orchestrator, then its workers, those waiting on you first. The one you are talking to is inked. A running worker's bar streams; a worker waiting on you shows its question and an **Answer** button. Click a session again to fold this pane away.
...
Drag the gap between two panes to resize them; double-click it to restore the default width. ⌘1, ⌘2 and ⌘3 fold the sessions (to a rail of initials), workers and changes panes, as do the three toggles in the title bar. Widths and folds are remembered per repository.
...
| A session's **⋯** | Rename; for the open session, its brief, expanded reasoning (⌘O), clear the view, trim the transcript; shut down its orchestrator; remove it with its workers, after a confirmation |
...
| The waiting strip | **Review** a pending approval or model choice; **Answer** a worker's question |
