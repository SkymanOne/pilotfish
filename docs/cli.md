...
If Jev's confidence is below `confidence_threshold`, the choice is referred to you. The console raises a prompt listing every candidate, Jev's preference first, with its probability and price. `enter` or `1` to `9` selects one, and `d` keeps the configured model. The spawn, and with it the orchestrator's `fleet_spawn` call, waits for the answer. Without an answer within ten minutes (`$PILOTFISH_ASK_TIMEOUT_MS`), or when no console is open, the configured model is used.
